      ...
      if params[:event]
        ...
        if @event.save
          ...
          if params[:event][:avatar]
            content_type = Mime::Type.lookup_by_extension(File.extname(params[:event][:avatar])[1..-1]).to_s
            image_file = Paperclip.io_adapters.for("data:#{content_type};base64,#{params[:event][:avatar]}")
            image_file.original_filename = params[:event][:avatar]
            @event.update_attributes(:avatar => image_file)
          end
              ...
              def update_event
                if !@event.blank?
                  ...
                  def destory
                    ...
                    def upcoming_events
                      @events  = Event.where("start_time >= ?", Date.today).limit(10)
                      if @events.present?
                        render :events
                      else
                        render :json => {
